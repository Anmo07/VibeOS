...
class LiveImageCreatorBase(LoopImageCreator):
    ...
    def _generate_efiboot(self, isodir):
        ...
        efi_dir = isodir + "/EFI/BOOT"
        print(f"DEBUG: _generate_efiboot called for {isodir}")
        if not os.path.exists(efi_dir):
            print(f"DEBUG: EFI directory {efi_dir} NOT found!")
            return

        efi_files = glob.glob(efi_dir + "/BOOT*.EFI")
        if not efi_files:
            print(f"DEBUG: No BOOT*.EFI files found in {efi_dir}! (Target was {self.efiarch})")
            return
        
        print(f"DEBUG: Found {len(efi_files)} EFI files: {efi_files}")

        target_img = isodir + "/isolinux/efiboot.img"
        print(f"DEBUG: Running mkefiboot {efi_dir} {target_img}")
        # XXX-BCL: does this need --label?
        rc = subprocess.call(["mkefiboot", efi_dir, target_img])
        print(f"DEBUG: mkefiboot returned {rc}")
        if os.path.exists(target_img):
            print(f"DEBUG: Successfully created {target_img} (size: {os.path.getsize(target_img)} bytes)")
        else:
            print(f"DEBUG: FAILED to create {target_img}")
...
class x86LiveImageCreator(LiveImageCreatorBase):
    ...
    def _get_xorrisofs_options(self, isodir):
        print(f"DEBUG: Generating xorrisofs options for {self.fslabel}")
        isohdpfx = "/usr/share/syslinux/isohdpfx.bin"
        if not os.path.exists(isohdpfx):
            # Check instroot or alternative locations
            alt_paths = [
                self._instroot + "/usr/share/syslinux/isohdpfx.bin",
                self._instroot + "/usr/lib/syslinux/isohdpfx.bin",
        ...
        print(f"DEBUG: Using isohdpfx at {isohdpfx}")

        options = ["-eltorito-boot", "isolinux/isolinux.bin",
                   "-no-emul-boot", "-boot-info-table", "-boot-load-size", "4",
                   "-eltorito-catalog", "isolinux/boot.cat",
                   "-isohybrid-mbr", isohdpfx]
        dirs = ["isolinux", "images"] # Prefer isolinux for consistency with mkefiboot
        for img in (("efiboot.img", "basdat"), ("macboot.img", "hfsplus")):
            for d in dirs:
                img_path = os.path.join(isodir, d, img[0])
                if os.path.exists(img_path):
                    print(f"DEBUG: Found boot image {img[0]} at {img_path}")
                    options += ["-eltorito-alt-boot", "-e", d + "/" + img[0],
                                "-no-emul-boot", "-isohybrid-gpt-" + img[1]]
                    if img[0] == "efiboot.img":
                        options += ["-partition_offset", "16",
                                    "-append_partition", "2", "0xef", img_path,
                                    "-appended_part_as_gpt"]
                    dirs = [d]
                    break
                else:
                    print(f"DEBUG: Boot image {img[0]} NOT found at {img_path}")
...
"""
    ...
    def efiarch(self):
        if not self._efiarch:
            ...
            efiarch = {"i386": "IA32", "x86_64": "X64", "aarch64": "AA64"}
    ...
    def __copy_efi_files(self, isodir):
        ...
        """
        fail = False
        # Extended search paths for EFI files on various Fedora/RHEL versions
        search_paths = [
            "/boot/efi/EFI/*/shim%s.efi" % (self.efiarch.lower(),),
            "/boot/efi/EFI/fedora/shim%s.efi" % (self.efiarch.lower(),),
            "/usr/share/shim/%s/shim%s.efi" % (self.efiarch.lower(), self.efiarch.lower()),
            "/boot/efi/EFI/*/gcd%s.efi" % (self.efiarch.lower(),),
            "/boot/efi/EFI/fedora/gcd%s.efi" % (self.efiarch.lower(),),
            "/boot/efi/EFI/*/grub%s.efi" % (self.efiarch.lower(),),
            "/boot/efi/EFI/fedora/grub%s.efi" % (self.efiarch.lower(),),
            "/boot/efi/EFI/*/shimia32.efi",
            "/boot/efi/EFI/*/gcdia32.efi",
        ...
        files = [("/boot/efi/EFI/*/shim%s.efi" % (self.efiarch.lower(),), "/EFI/BOOT/BOOT%s.EFI" % (self.efiarch,), True),
                 ...
                 ("/boot/efi/EFI/*/grub%s.efi" % (self.efiarch.lower(),), "/EFI/BOOT/grub%s.efi" % (self.efiarch.lower(),), False),
                 ("/boot/efi/EFI/*/shimia32.efi", "/EFI/BOOT/BOOTIA32.EFI", False),
                 ("/boot/efi/EFI/*/gcdia32.efi", "/EFI/BOOT/grubia32.efi", False),
                 ("/usr/share/grub/unicode.pf2", "/EFI/BOOT/fonts/unicode.pf2", False),
                 ("/boot/grub2/fonts/unicode.pf2", "/EFI/BOOT/fonts/unicode.pf2", False),
                ]
        makedirs(isodir+"/EFI/BOOT/fonts/")
        
        # Helper to find and copy
        def find_and_copy(src_pattern, dest_path, required):
            # Try original instroot path
            matches = glob.glob(self._instroot + src_pattern)
            if not matches:
                # Try some common alternative paths
                alt_paths = [
                    "/boot/efi/EFI/fedora/" + os.path.basename(src_pattern),
                    "/boot/efi/EFI/BOOT/" + os.path.basename(src_pattern),
                    "/usr/share/shim/" + self.efiarch.lower() + "/" + os.path.basename(src_pattern),
                    "/usr/lib/grub/" + self.efiarch.lower() + "-efi/" + os.path.basename(src_pattern),
                    "/usr/lib/grub/" + os.path.basename(src_pattern),
                    "/boot/grub2/fonts/" + os.path.basename(src_pattern),
                    "/usr/share/grub/" + os.path.basename(src_pattern)
                ]
                for alt in alt_paths:
                    matches = glob.glob(self._instroot + alt)
                    if matches: break
            
            if not matches:
                # Last resort: find anywhere in instroot
                print(f"DEBUG: Searching everywhere for {os.path.basename(src_pattern)}...")
                for root, dirs, files in os.walk(self._instroot):
                    if os.path.basename(src_pattern) in files:
                        matches = [os.path.join(root, os.path.basename(src_pattern))]
                        break

            if matches:
                print(f"DEBUG: Found {src_pattern} at {matches[0]}")
                shutil.copy(matches[0], isodir + dest_path)
                return True
            return False

        for src, dest, required in files:
            if not find_and_copy(src, dest, required):
                if required:
                    logging.error("Missing EFI file (%s)" % (src,))
                    fail = True
        
        # Ensure unicode.pf2 exists
        if not glob.glob(isodir + "/EFI/BOOT/fonts/unicode.pf2"):
            # Try to find it anywhere in instroot
            font_matches = glob.glob(self._instroot + "/**/unicode.pf2", recursive=True)
            if font_matches:
                shutil.copy(font_matches[0], isodir + "/EFI/BOOT/fonts/unicode.pf2")
            else:
                logging.error("Missing unicode.pf2 font file!")
                fail = True
...
""" %args
    ...
    def __get_efi_image_stanza(self, **args):
        if self._isDracut:
            ...
            args["rootlabel"] = "CDLABEL=%(fslabel)s" % args
        return """menuentry '%(long)s' --class custom --class gnu-linux --class gnu --class os {
...
"""
    ...
    def _configure_efi_bootloader(self, isodir):
        ...
        if self.__copy_efi_files(isodir):
            # shutil.rmtree(isodir + "/EFI") # Don't delete if we failed some parts
            logging.warning("Failed to copy all EFI files, EFI Support might be broken.")
            # return # Try to continue anyway
    ...
    def _get_xorrisofs_options(self, isodir):
        ...
        if os.path.exists(os.path.join(isodir, "isolinux/efiboot.img")):
            options += ["-eltorito-alt-boot", "-e", "isolinux/efiboot.img",
                        "-no-emul-boot", "-isohybrid-gpt-basdat", "-hide-rr-moved",
                        "-partition_offset", "16",
                        "-append_partition", "2", "0xef", os.path.join(isodir, "isolinux/efiboot.img"),
                        "-appended_part_as_gpt"]
        options += ["-rational-rock", "-joliet", "-volid", self.fslabel]
        return options
